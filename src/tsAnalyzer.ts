        ...
        switch (s.type) {
            ...
            case "VariableDeclaration": {
                ...
                for (const d of (s as VariableDeclaration).declarations) {
                    ...
                    addNode(nodeId, lbl, "rect", d, 'fill:#ffffff,stroke:#424242,stroke-width:2px');
            ...
            case "ExpressionStatement": {
                ...
                if (e.type === "CallExpression") {
                    ...
                    if (isConsoleLog(ce)) {
                        ...
                        addNode(nodeId, lbl, "para", s, 'fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px');
                    } else {
            ...
            case "IfStatement": {
                ...
                addNode(condId, condLbl, "diamond", ifs, 'fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px');
                if (current) {addEdge(current, condId);}

                // YES branch
                current = condId;
                const yesHead = emitBlockFirst(ifs.consequent);
                if (yesHead) {fixLastEdgeLabel(condId, "Yes");}
                ...
                if (ifs.alternate) {
                    ...
                    if (noHead) {fixLastEdgeLabel(condId, "No");}
                    noTail = collectTail();
                } else {
                ...
                if (yesTail) {branchEnds.push(yesTail);}
                if (noTail) {branchEnds.push(noTail);}
            ...
            case "ReturnStatement": {
                ...
                const nodeId = nextId();
                const lbl = r.argument ? "return " + labelFromRange(r.argument) : "return";
                addNode(nodeId, lbl, "round", s, 'fill:#ffebee,stroke:#b71c1c,stroke-width:2px');
                if (current) {addEdge(current, nodeId);}
            ...
            case "WhileStatement": {
                ...
                addNode(loopId, header, isWhile ? "diamond" : "rect", s, 'fill:#e3f2fd,stroke:#0d47a1,stroke-width:2px');

                if (current) {addEdge(current, loopId);}
                loopStack.push(loopId);

                // body
                current = loopId;
                const bodyFirst = emitBlockFirst(isWhile ? (s as WhileStatement).body : (s as ForStatement).body);
                if (isWhile && bodyFirst) {fixLastEdgeLabel(loopId, "True");}

                const bodyTail = collectTail();
                if (bodyTail && bodyTail !== loopId) {addEdge(bodyTail, loopId);} // back edge
                loopStack.pop();
